...
- [x] T043 [P] Pin all dependencies for reproducible builds (constitution v1.3.1, Principle I): no floating tags — base images (`python`, `node`, `postgres`, `rabbitmq`) and `uv` pinned to explicit **patch version + digest** across `backend/Dockerfile`, `frontend/Dockerfile`, `docker-compose.yml`, and `tests/conftest.py`; deployed image referenced immutably (git-SHA tag) via `deploy/k8s/deployment.yaml` + CD `kubectl set image`; GitHub Actions pinned to commit SHAs (version in trailing comment)
