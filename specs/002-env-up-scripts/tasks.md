...
- [X] T030 Run the [quickstart.md](./quickstart.md) validation scenarios 1–8 (dev bring-up, idempotent re-run, run-from-subdirectory, prod fail-fast, wrapper exit-code propagation, runtime image has no `uv`, migrations once-per-deploy, LF guard). **Validated here**: Scenario 6 (image built; `NO_UV`, app imports, ~80 MB), Scenario 8 (`check-attr` text/eol=lf, 0 CR bytes), Scenario 5 (wrappers thin: WSL guard + `wsl.exe` call + `exit $LASTEXITCODE`). Scenarios 1/2/7 validated at manifest level (`kubectl kustomize` → kubeconform v0.8.0: base 9/9, dev 15/15, prod 9/9; both Docker images build). **Deferred** (need a live cluster — `kind` not installed in this env): live 1–4, 7 end-to-end bring-up/rollout. Existing pytest suite re-run green (15 passed) after the conftest fixture move.
